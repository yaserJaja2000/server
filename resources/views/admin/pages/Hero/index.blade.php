...
@section('content')
    <div class="card shadow-lg">
        <div class="card-header d-flex align-items-center justify-content-between fw-bold">
            <h5 class="m-0 font-weight-bold text-secondary"> السلايدر الرئيسي</h5>
            <a href="{{ route('hero.create') }}" class="btn btn-sm btn-primary">
                    ...
                    @foreach ($lists as $list)
                        <tr>
                            <td class="align-middle">
                                <img src="{{ asset($list->image) }}" width="100" alt="">
                            </td>
                            <td class="align-middle">{{ $list->title }}</td>
                            <td class="align-middle">{{ substr($list->description, 0, 25) }}...</td>
                            <td class="align-middle">
                                <a href="{{ route('hero.show', $list->id) }}" class="btn btn-sm btn-info mb-1">
                                    <i class="bi bi-eye-fill"></i>
                                    {{-- مشاهدة --}}
                                </a>
                                <a href="{{ route('hero.edit', $list->id) }}" class="btn btn-sm btn-success mb-1">
                                    <i class="bi bi-pencil-square"></i>
                                    {{-- تعديل --}}
                                </a>
                                <button class="btn btn-sm btn-danger delete-btn mb-1" data-bs-toggle="modal"
                                    data-bs-target="#deleteDeal" data-action="{{ route('hero.destroy', $list->id) }}"
                                    onclick="setAction(this.dataset.action)"
                                    data-action="{{ route('hero.destroy', $list->id) }}">
